    ...
    *,
    ...
    for _, item in packet.iterrows():
        ...
        if route == "Price dry-run path":
            ...
            post_run = f"make readiness-snapshot PROFILE={selected_profile} && make price-validate && make price-preview && make price-apply && make reviewed-batch-compare PROFILE={selected_profile} LANE=prices BATCH_ID=<reviewed_batch_id> REVIEW_DATE=<yyyy-mm-dd> && make stock-report-md TICKER={selected[0]}"
        elif route == "SEC-stageable":
            ...
            post_run = f"make readiness-snapshot PROFILE={selected_profile} && make imports-validate IMPORT_TICKERS={selected[0]} && make imports-preview IMPORT_TICKERS={selected[0]} && make imports-apply IMPORT_TICKERS={selected[0]} && make dcf-readiness && make reviewed-batch-compare PROFILE={selected_profile} LANE=fundamentals BATCH_ID=<reviewed_batch_id> REVIEW_DATE=<yyyy-mm-dd> && make stock-report-md TICKER={selected[0]}"
        else:

...
REVIEWED_BATCH_OUTCOME_REQUIRED_FIELDS: tuple[OutcomeRequiredField, ...] = (
    ...
    OutcomeRequiredField(
        ...
        f"make reviewed-batch-compare PROFILE={resolve_readiness_proof_profile()} LANE=<lane> BATCH_ID=<reviewed_batch_id> REVIEW_DATE=<yyyy-mm-dd>",
    ),
    OutcomeRequiredField(
        "changed_tickers",
        "Changed Tickers",
...
PROOF_COMPLETION_ACTIONS: dict[str, str] = {
    ...
    "changed_readiness_counts": f"Run make readiness-snapshot PROFILE={resolve_readiness_proof_profile()} and make reviewed-batch-compare PROFILE={resolve_readiness_proof_profile()} LANE=<lane> BATCH_ID=<reviewed_batch_id> REVIEW_DATE=<yyyy-mm-dd>, then copy the count delta or none.",
    "changed_tickers": f"Run make reviewed-batch-compare PROFILE={resolve_readiness_proof_profile()} LANE=<lane> BATCH_ID=<reviewed_batch_id> REVIEW_DATE=<yyyy-mm-dd>, then copy changed tickers or none.",
    ...
    *,
    ...
    for item in REVIEWED_BATCH_OUTCOME_REQUIRED_FIELDS:
        ...
        if item.field in {"changed_readiness_counts", "changed_tickers"} and comparison_status != "ok":
            ...
            copy_from = f"make readiness-snapshot PROFILE={resolve_readiness_proof_profile()}, then make reviewed-batch-compare PROFILE={resolve_readiness_proof_profile()} LANE=<lane> BATCH_ID=<reviewed_batch_id> REVIEW_DATE=<yyyy-mm-dd>"
    ...
    *,
    ...
    for row in blocked_rows:
        ...
        if validation_status == "blocked_by_snapshot_gate":
            action = f"Run make readiness-snapshot PROFILE={resolve_readiness_proof_profile()}, then make reviewed-batch-compare PROFILE={resolve_readiness_proof_profile()} LANE=<lane> BATCH_ID=<reviewed_batch_id> REVIEW_DATE=<yyyy-mm-dd> before copying changed readiness proof."

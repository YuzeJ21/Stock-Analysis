    ...
    *,
    ...
    if not requested_tickers:
        return {
            ...
            "unresolved_tickers": [],
            "rows": [],
            "warnings": ["No tickers were provided for SEC staging workflow."],
        }
...
def write_sec_fundamentals_import(
    ...
    if output.name != "fundamentals.csv" or output.parent.name != "imports":
        raise ValueError("SEC fundamentals staging workflow may only write to a fundamentals.csv file inside a data/imports directory.")

    canonical_like = output.parent.parent / "fundamentals.csv"
    if output.resolve() == canonical_like.resolve():
        raise ValueError("SEC fundamentals staging workflow must not write directly to canonical data/fundamentals.csv.")
    ...
    if output.exists() and not overwrite:
        ...
        if existing_validation.status == "invalid":
            raise ValueError("Existing fundamentals import file is invalid. Fix or remove it before SEC staging workflow.")

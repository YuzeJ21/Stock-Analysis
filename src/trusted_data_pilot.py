...
def render_trusted_data_pilot_candidates(candidates: list[PilotCandidate], *, top_n: int = DEFAULT_TOP_N) -> str:
    ...
    lines.extend(
        [
            ...
            f"2. make trusted-data-pilot TICKERS={ticker_list} TOP_N={len(candidates)}",
            f"3. make stock-report-md TICKER={first}",
            f"4. {candidates[0].next_command}",
            "5. make imports-validate && make imports-preview && make imports-apply",
            "6. make readiness && make dcf-readiness",
            f"7. make stock-report-md TICKER={first}",
            "",

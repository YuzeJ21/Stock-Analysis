...
def build_onboarding_actions(coverage_rows: list[TickerCoverage]) -> list[OnboardingAction]:
    ...
    actions.append(
        OnboardingAction(
            ...
            recommended_action=(
                "Run make universe-preview-summary first. Fill data/custom_universe.csv with verified tickers only if every "
                "remote SMH source is unavailable."
            ),
            target_file="data/imports/universe.csv",
            focus_command="make universe-preview-summary",
            example_command="make universe-preview-summary",
        )
    )
...
def _print_coverage(payload: dict[str, Any], *, top_n: int | None = None) -> None:
    ...
    print(f"Missing-data steps: {len(payload['onboarding_actions'])}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Inspect local ticker-level coverage and missing-data steps.")

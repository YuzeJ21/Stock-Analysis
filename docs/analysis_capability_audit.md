...
| Single-stock report | Clearest visitor-facing review. | See one ticker's At A Glance mode, Best Review Path, ready, blocked, excluded, optional, methodology, copyable unlock commands, and source readiness states step by step. | Current local readiness, price, decision, DCF, peer, and optional-context outputs. | It does not run imports or refreshes, connect to external accounts, provide allocation instructions, or issue recommendations. | `src/stock_report.py`, `src/dashboard.py` |

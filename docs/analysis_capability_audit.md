...
- **Single-stock report:** one ticker can be reviewed with At A Glance status, Evaluation Snapshot, Proof Checklist, Best Review Path, readiness, supported analysis, blocked analysis, valuation state, methodology, risk notes, copyable local proof commands, next step, and source readiness notes.
...
| Single-stock report | Clearest visitor-facing review. | See one ticker's At A Glance mode, Evaluation Snapshot, Proof Checklist, Best Review Path, ready, blocked, excluded, optional, methodology, copyable proof commands, and source readiness states step by step. | Current local readiness, price, decision, DCF, peer, and optional-context outputs. | It does not run imports or refreshes, connect to external accounts, provide allocation instructions, or issue recommendations. | `src/stock_report.py`, `src/dashboard.py` |

...
- Read-only SEC quarterly cash-generation source preview: `make sec-quarterly-cash-preview AS_OF=<timezone-aware-cutoff>` fetches only exact Companyfacts, submissions, and primary-filing endpoints in memory. NVIDIA Q1 FY2027 accession `0001045810-26-000052` passed with exact UTC acceptance time and explicit filed-table capex outflow proof; it created no generated artifact and did not activate Company Workbench or readiness.
- Explicit Company Workbench cash-generation preview: only the opt-in `cash_preview=1` NVIDIA route composes the accepted SEC result at the fixed review cutoff. It uses complete withholding, labels every value preview-only rather than ready, keeps technical lineage under Advanced, leaves `production_activation=false` and `readiness_promotions=()`, makes no canonical persistence or generated artifact, and does not alter the normal Workbench route.
...
- Quarterly cash-generation source adapter: `one_company_source_preview_accepted_for_review`; NVIDIA Q1 FY2027 passed the exact SEC payload, acceptance-time, field-scope, inline-fact, capex-sign, and adapter contracts. Production activation, another-company portability, historical depth, and Q4 coverage remain unproven; do not repeat the NVIDIA pilot unless its source filing changes or a regression is suspected.
- Quarterly adapter activation: `explicit_preview_only_composition_implemented`; one bounded Company Workbench route now renders the accepted NVIDIA result without production activation, persistence, readiness promotion, or normal-route changes. This is not canonical evidence activation.
- Quarterly cash-generation portability: `bounded_second_company_source_selection_required`; the next evidence-depth assessment is one bounded second-company proof with an exact eligible filing, explicit source identity, reviewed field scope, acceptance timestamp, filed-table capex-sign evidence, and the same fail-closed contract. Do not repeat the NVIDIA pilot, run broad coverage, infer a company, or start sourcing loops without an exact justified candidate.

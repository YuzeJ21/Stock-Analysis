...
- Personal Research Mode with Research Desk -> Discover -> Company Workbench -> Monitor. Discover presents the unchanged strict Daily Momentum & Valuation contract as **Screen eligibility — when supported**, then separately provides alphabetical **Browse saved companies** evidence access from focused-cohort ticker readiness. Saved rows answer why they are inspectable, what evidence is usable, and the main evidence gap; browsing is not screen eligibility, opportunity ranking, expected return, or recommendation. The research path never reads legacy decisions, rankings, or watchlist outputs and writes no artifacts.
...
Current local product slice awaiting release closure: Discover truth separation and the Company Workbench primary brief are implemented test-first. Strict eligibility remains empty when required evidence is incomplete and never relaxes thresholds. Alphabetical saved-company browsing remains available from readiness-only identities without importing ranking-adjacent fields or legacy outputs. Workbench now shows one Company Brief with usable evidence, withheld evidence, change state, one authoritative next task, the stop rule, and a ticker-bound Data Health action before any detailed module. One explicit session-local action restores the unchanged trend, valuation, scenario, authoring, methodology, conclusion-detail, Research Decision Lab, and offline HTML-brief surfaces. It does not write data, change readiness, or duplicate the primary task. Exact local runtime anchor `ad431eadbd00df71419f91e3a18408b0afeb94e4` passed the clean six-route browser matrix at `1280x720` and `390x844`. Release anchor `35b355a5921bd7d7e9f3c46ab28a799f9e42818e` passed 6,323 tests, dashboard/render/public checks, and 60/60 performance checks; pilot readiness remained truthfully blocked on working readiness and external source proof. Complete push, draft-PR update, and exact-head CI evidence before calling this slice complete. Then execute **Monitor consolidation** as the next separate test-first slice; Research Desk simplification and shared-shell cleanup follow. The next external data unblock remains one bounded permitted price-history plus historical-valuation evidence package through separately reviewed source activation.

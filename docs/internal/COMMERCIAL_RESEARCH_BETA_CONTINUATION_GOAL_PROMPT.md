...
- Evidence-quality lineage anchor: commit `7ba5e7612` or a later verified descendant.
...
- Pull-request engineering gate: `.github/workflows/commercial-research-beta.yml` runs the full tests plus the minimal dashboard, Personal Research render, wording, hygiene, and whitespace contract only for pull requests to `main`, with `contents: read` and no provider, readiness, deployment, secret, schedule, or artifact-upload path. Direct GitHub evidence passed on the hosted-verification lineage after clean-environment corrections; reverify every current revision, and automation never substitutes for human review.

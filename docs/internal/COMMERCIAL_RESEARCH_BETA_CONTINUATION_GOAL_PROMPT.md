...
- Performance sampling reconciliation is present in the working product tree after `1b73f1221d82c85092c21fe95540aa67452c9f72`: warm shell/first-useful p90 and cold shell/first-useful maximum are separate and independently enforce the unchanged one-second and three-second limits. Reverify the commit and remote lineage because the current managed session cannot write Git metadata. Its focused unit contract passed, but Chrome could not launch in the managed sandbox; this is not a performance pass. The next normal-terminal operator action is one `make commercial-beta-performance-gate TIMEOUT_SECONDS=90` run, followed by `make commercial-beta-release-check` only if the measured gate passes. Accept category-specific failures without an unchanged retry loop.

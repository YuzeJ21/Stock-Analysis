...
- Research Decision Lab approved-design anchor: commit `54e06e1c3` or a later verified descendant; read `docs/superpowers/specs/2026-07-22-research-decision-lab-design.md` and treat it as the current highest-value local product stage until every acceptance criterion has direct implementation evidence. The design commit proves no product behavior by itself.
...
- Research Decision Lab is approved and planned, not implemented at the design anchor. It must compose the existing journal, scorecard, scenario, outcome, report, and Change Monitor contracts without adding a route, ledger, readiness state, transaction field, position sizing, stop/profit rule, recommendation, or broker action. Do not list it as a current capability until its focused/full tests, responsive runtime review, release gates, exact staging, push, draft-PR update, and exact-head CI pass.
...
5. Unless current code and tests directly prove every Research Decision Lab acceptance criterion, treat `docs/superpowers/specs/2026-07-22-research-decision-lab-design.md` as the next local product priority. Create its detailed implementation plan if missing, then execute the read-only composition, Company Workbench integration, Monitor discipline review, and documentation/release slices in order. If it is already complete or genuinely locally blocked, identify the next highest-impact executable usability, methodology, evidence, reliability, or operating-readiness gap.
...
Stage A-G labels are continuation maturity lanes only; they do not replace the numbered Stage 0-6 exit gates below. Stage A is the implemented isolated prospective field-proof primitive. The approved Research Decision Lab is now the highest-value local product lane. Stage B — local field-proof audit and operator hardening remains a separate safe operator lane after the Decision Lab is complete or genuinely locally blocked: improve read-only audit, error explanation, and operator review ergonomics only, with no readiness mapping. Mapping field proof into proof-readiness reconciliation, Company Workbench, canonical data, or any readiness lane requires a separate design and approval. Later lettered lanes remain planning labels until separately designed; they grant no release-stage exit.

...
- Focused-cohort adjusted-price review: saved price readiness is necessary but not sufficient in Commercial Research mode; every retained canonical history row needs technical date/positive-close validity, exact `source`/`source_ref`/`retrieved_at` provenance, approved exact-source rights, and registered `prices` scope. Mixed or unlined history fails closed. Canonical quarterly Revenue/EPS remain the open Priority 2 audit lane.
...
- Cohort saved-row or adjusted-price scope passage is display eligibility for one exact lane only. It cannot validate the payload or retrieval chronology, approve a source, change readiness, activate consensus, establish quarterly actual integrity, promote a peer role, or grant another lane. Canonical quarterly Revenue/EPS still require independent enforcement.

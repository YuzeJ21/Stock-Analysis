...
Current local product slice awaiting release closure: Discover truth separation is implemented test-first. Strict eligibility remains empty when required evidence is incomplete and never relaxes thresholds. Alphabetical saved-company browsing remains available from readiness-only identities, explains inspectability, usable evidence, and the main gap, and opens one Company Brief without importing ranking-adjacent fields or legacy output frames. Complete current-head full, browser, release, hygiene, push, draft-PR, and exact-head CI evidence before calling the slice complete. Then execute the approved **Company Workbench primary brief** as a separate test-first plan; Monitor consolidation and Research Desk simplification follow. The next external data unblock remains one bounded permitted price-history plus historical-valuation evidence package through separately reviewed source activation.

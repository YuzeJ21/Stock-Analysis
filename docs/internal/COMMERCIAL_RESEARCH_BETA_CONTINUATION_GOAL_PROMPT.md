...
- Stale readiness continuation gate across project status, provider setup, coverage frontier, Auto-Refresh Status, its runbook, and the commercial-beta release path: while readiness is stale or incomplete, `make readiness-preview TOP_N=20` is the only continuation-safe command; provider, scheduled-operation, and ranked coverage details remain planning context only, and `make readiness` is a separate intentional reviewed write.

...
- Quarterly adapter activation: `local_activation_design_required`; the preview does not supply Company Workbench production observations or alter readiness. The next safe design must preserve per-metric readiness, no persistence by default, exact source lineage, and an explicit rollback/withhold state. A second-company portability proof is an independent alternative next slice; do not run broad coverage.

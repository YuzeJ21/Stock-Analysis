...
- Run `make dashboard` locally and open `http://localhost:8501/?mode=public` to show the clean visitor workflow: Home readiness snapshot -> Stock Selector -> Single-Stock Report -> Data Health lane answer -> Proof History evidence. Turn off Public visitor mode only when you want Operator context, coverage frontier details, reviewed batch packets, and validate / preview / apply guidance.

...
The auto-refresh plan is scheduler-ready but still gate-first. Use `make auto-refresh-status SCHEDULE=daily` first when you want the current source activation state, setup gaps, avoid-repeat lane, next executable command, and runbook pointer in one read-only summary. Use `make auto-refresh-runbook SCHEDULE=daily`, `SCHEDULE=weekly`, or `SCHEDULE=optional` when you want a compact unattended checklist instead of the full policy dump. `auto_supported` means the deterministic gate passed validation, preview, rejected-row, source-provenance, scope, and no-fabrication checks. `human_reviewed_supported` means a person reviewed the evidence. `candidate_context_only` can route research, especially peer candidates, but it is not trusted proof. Rows that fail a source path should be recorded as `still_blocked`, `skipped`, or `excluded` and the workflow should pivot instead of retrying the same unavailable provider.

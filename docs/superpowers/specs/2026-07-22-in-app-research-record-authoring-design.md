...
The user approved the unified Company Workbench composer design on 2026-07-22. This written specification is awaiting the required pre-implementation review. No production implementation is authorized by this document alone.
...
- The actual UTC recording timestamp is captured when validation creates the preview and is shown in that preview. Confirmation persists that exact timestamp; it does not silently replace it.
- User-entered effective, publication, retrieval, review, and observation timestamps remain explicit inputs and must satisfy the existing temporal validators.

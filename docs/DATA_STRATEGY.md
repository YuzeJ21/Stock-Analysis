...
The stale readiness continuation gate applies that boundary consistently to project status, Session Source Preflight, provider setup, the coverage frontier, Auto-Refresh Status, its runbook, and Advanced Data Health cards; the same gate also covers unverified or uncommitted working readiness evidence. While readiness is stale, incomplete, unverified, or present only as uncommitted working evidence, `make readiness-preview TOP_N=20` is the only continuation-safe command. Source availability, provider availability, scheduled operations, and ranked opportunities remain planning context only; their source, refresh, apply, proof, and rebuild commands are not executable routing. `make readiness` is a separate intentional reviewed write after source review and generated-artifact review.

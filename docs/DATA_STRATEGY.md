...
The stale readiness continuation gate applies that boundary consistently to project status, provider setup, and the coverage frontier. While readiness is stale or incomplete, `make readiness-preview TOP_N=20` is the only continuation-safe command. Provider availability and ranked opportunities remain planning context only; their source, refresh, apply, proof, and rebuild commands are not executable routing. `make readiness` is a separate intentional reviewed write after source review and generated-artifact review.

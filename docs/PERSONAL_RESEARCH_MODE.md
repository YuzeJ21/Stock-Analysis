...
Canonical query allowlists strip unsupported route state before rendering, so parameters from another workflow cannot silently activate a different Personal route. Operator mode keeps Operator Data Health links in Operator mode, while Personal Research uses its mode-specific read-only evidence language and preserves an explicitly selected peer lane when handed off from Company Workbench.

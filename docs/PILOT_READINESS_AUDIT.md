...
Last repo-truth refresh: 2026-07-01
...
| Test coverage | Green | Full test suite passes locally in the current package: `1932 passed, 1 warning`. |
...
| PB-008 | Keyed free-tier providers not configured | Medium | More fundamentals/share-count fallback coverage cannot expand through FMP, Alpha Vantage, or Finnhub until local keys are configured. | `make session-source-preflight` reports FMP, Alpha Vantage, and Finnhub as `provider_key_missing`; `make project-status` says the best next proof is `make provider-setup-checklist`. | No, owner setup dependent | Keep real keys out of the repo; use provider smoke commands before broad batches after setup. | External/manual gate |

## Command Evidence

| Command | Result | Notes |
| --- | --- | --- |
| `git status --short --branch --untracked-files=no` | Passed with local commits and generated churn | Current branch is ahead of origin with reviewed local commits; dirty files are generated CSV/report churn and broad stock-report artifacts that stay excluded by default. |
| `git log -5 --oneline` | Passed | Latest commits include provider setup and pilot share brief smoke-command improvements. |
| `make diff-hygiene-summary` | Passed | Product/code/docs/test package is clean before this audit refresh; generated/sample report churn stays local unless individually reviewed evidence. |
...
- Provider setup: FMP, Alpha Vantage, and Finnhub are keyed free-tier fallbacks. Configure them only in local env files, run the provider smoke command first, then validate and preview before applying any row.

...
- Data Health lane answer should stay the first coverage-readiness surface: one answer per lane before queue drawers, route maps, raw tables, or proof ledgers.
- Proof History evidence is the public proof-inspection surface. `Inspect proof` should land there rather than shortcutting visitors into a generic Data Health table.
...
- Confirm provider setup remains source-gated: No broad coverage batch should run from setup alone. Provider setup only makes a source executable; readiness changes still require validate, preview, rejected-row review, source provenance, apply/skip decision, rebuilt readiness, and proof ledger evidence. Do not retry exhausted proof queues until new source-backed rows, keyed provider data, reviewed manual rows, or changed blockers exist.

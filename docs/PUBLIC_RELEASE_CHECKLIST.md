...
- Confirm provider setup remains source-gated: a configured key or optional read-only provider only makes a source executable; it does not unlock readiness until validate, preview, rejected-row review, apply/skip decision, rebuilt readiness, and proof ledger evidence pass.

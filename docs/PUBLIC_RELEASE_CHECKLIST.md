...
- Confirm local file status is framed as inspection only: file presence, row counts, staged-folder counts, or rejected-row report existence are not proof until source review, validation, preview/apply, readiness rebuild, and the regenerated report prove the lane changed.

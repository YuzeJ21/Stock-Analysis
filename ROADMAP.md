...
22. **Implemented locally:** the stale readiness continuation gate now gives project status, provider setup, the coverage frontier, Auto-Refresh Status, its runbook, and the commercial-beta release path one fail-closed operator answer. While selected-profile readiness is stale or incomplete, `make readiness-preview TOP_N=20` is the only continuation-safe command; provider classifications, scheduled operations, and ranked coverage rows remain planning context only, and `make readiness` remains a separate intentional reviewed write.

...
38. **Implemented locally:** focused-cohort adjusted price history now requires saved price readiness plus technically usable canonical rows, complete row-level `source`/`source_ref`/`retrieved_at` provenance, approved exact-source rights, and registered `prices` scope before Commercial Research coverage is usable. Every retained history row must pass; mixed or unlined history fails closed. Current canonical history remains useful for research but blocked commercially because it has no row lineage. Priority 2 remains open for canonical quarterly Revenue/EPS packet enforcement.

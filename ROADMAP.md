...
22. **Implemented locally:** the stale readiness continuation gate now gives project status, provider setup, and the coverage frontier one fail-closed operator answer. While selected-profile readiness is stale or incomplete, `make readiness-preview TOP_N=20` is the only continuation-safe command; provider classifications and ranked coverage rows remain planning context only, and `make readiness` remains a separate intentional reviewed write.
...
The stale readiness continuation gate improves **operating reliability** by preventing status, provider, and coverage surfaces from routing an operator into broad source or refresh work from stale counts. It does not refresh data, prove source correctness, authorize a rebuild, or satisfy an external gate. It does not prove market validation.

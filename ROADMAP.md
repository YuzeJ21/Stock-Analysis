...
If the current branch head lacks direct local matrix, branch synchronization, draft-PR update, or exact-head CI evidence, complete those release steps first; otherwise select the first incomplete safe roadmap priority. If its next gate needs an unavailable source, account, environment, reviewer, or elapsed event history, classify it once under **Externally blocked** and continue to the next executable priority. Passing local tests never completes an external gate. Evidence publication, snapshot, and retrieval timestamps must all be at or before the cutoff.
...
Current answer-first UX program: Discover truth separation, the Company Workbench primary brief, Monitor consolidation, and Research Desk simplification are implemented locally. Research Desk replaces the weekly summary plus four overlapping answer cards with one Today's Research Brief and keeps supporting evidence under Advanced. Workbench composes **Use now**, **Still withheld**, **What changed**, one authoritative **Next research task**, the research-only stop rule, and the ticker-bound Data Health action before secondary modules. Trend, valuation, scenarios, authoring, methodology, the Research Decision Lab, conclusion detail, and the offline HTML brief remain available only after one session-local **Open evidence and analysis modules** action; reopening cannot change canonical data or readiness. Monitor replaces three competing primary summaries with one five-panel Follow-up Queue and one fail-closed zero-state action. The direct browser matrix passed all six routes at desktop and phone widths at implementation anchor `5a2aaf2a44dc7ae371161bb8642ab7f0755ad019`; it verified one Desk brief/action, no horizontal overflow, media-preference assertions, stable semantic-main behavior, and byte-for-byte repository state while excluding the same 18 generated paths. At local release-verification anchor `0e41811cde7d95f5d741da15fc4f72c337d6566f`, 6,334 tests, dashboard startup and six-route render smoke, public wording, 60/60 performance, `public-check`, pilot-readiness classification, hygiene, whitespace, and protected-artifact identity passed. Pilot readiness remains truthfully blocked on uncommitted working readiness evidence and incomplete source proof. This remains local engineering evidence, not hosted, independent-human, screen-reader, WCAG, or market validation. Push, draft-PR update, and exact-head CI still close this slice. The next executable local slice is **shared-shell and navigation cleanup** as a separate tested change. These presentation changes must not alter calculations, authoring persistence, readiness, evidence identities, thresholds, or source-rights decisions.
...
Priority 7 accessibility remains open. Select any remaining safe automated work only through the durable release-routing rule above; true zoom, forced colors, reduced motion, screen-reader, and independent-human evidence remain external until a suitable environment exists.

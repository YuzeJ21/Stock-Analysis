...
Current answer-first UX program: Discover truth separation and the Company Workbench primary brief are implemented locally. At exact local runtime anchor `ad431eadbd00df71419f91e3a18408b0afeb94e4`, the clean accessibility browser matrix passed all six routes at `1280x720` and `390x844`. At release anchor `35b355a5921bd7d7e9f3c46ab28a799f9e42818e`, 6,323 tests, dashboard/render/public checks, and 60/60 performance checks passed; pilot readiness remained truthfully blocked on working readiness and external source proof. Push, draft-PR update, and exact-head CI evidence remain to be closed before the slice is complete. Workbench now composes **Use now**, **Still withheld**, **What changed**, one authoritative **Next research task**, the research-only stop rule, and the ticker-bound Data Health action before secondary modules. Trend, valuation, scenarios, authoring, methodology, the Research Decision Lab, conclusion detail, and the offline HTML brief remain available only after one session-local **Open evidence and analysis modules** action; reopening cannot change canonical data or readiness. The next executable local slice is **Monitor consolidation**, followed by Research Desk simplification and shared-shell cleanup as separate tested slices. These presentation changes must not alter calculations, authoring persistence, readiness, evidence identities, thresholds, or source-rights decisions.

...
Current answer-first UX program: Discover truth separation, the Company Workbench primary brief, and Monitor consolidation are implemented locally. Workbench composes **Use now**, **Still withheld**, **What changed**, one authoritative **Next research task**, the research-only stop rule, and the ticker-bound Data Health action before secondary modules. Trend, valuation, scenarios, authoring, methodology, the Research Decision Lab, conclusion detail, and the offline HTML brief remain available only after one session-local **Open evidence and analysis modules** action; reopening cannot change canonical data or readiness. Monitor replaces three competing primary summaries with one five-panel Follow-up Queue and one fail-closed zero-state action. At local release-verification anchor `05173ab3d1d2ace16add1c190a1362ef4c39459a`, 6,328 tests, dashboard and six-route render smoke, public wording, 60/60 performance, `public-check`, and the clean 12-case desktop/phone accessibility browser matrix passed; Monitor had no overflow, rendered one responsive five-panel grid, retained complete Advanced identities, and the same 18 generated paths stayed classified and excluded. Pilot readiness remained truthfully blocked on working readiness evidence, source proof, and pending GitHub synchronization. Push, draft-PR update, and exact-head CI remain to close the slice. The next executable local slice is **Research Desk simplification**, followed by shared-shell cleanup as a separate tested slice. These presentation changes must not alter calculations, authoring persistence, readiness, evidence identities, thresholds, or source-rights decisions.

...
Company Workbench HTML Research Brief — **Local feature gate: blocked on bidirectional active/passive action sanitation.** The implementation exists locally, but the portable download is not release-complete until deterministic active/passive sanitation and the Task 4 full current-head matrix and exact-head CI pass. The portable HTML action-policy repair is the first executable local release-safety lane. Priority 7 remains open but is not the active next lane. The brief downloads an immutable offline view of existing saved evidence and prepared Python scenario math, preserves independent field gates and research-only wording, writes no repository artifact, and does not activate readiness or create a new calculation engine. Source rights, current data, hosted operation, human and screen-reader accessibility, independent workflow sessions, screening validation, and probability calibration remain open gates. Release completion does not establish screening alpha, market fit, or investment validity. Local engineering evidence does not establish source rights, current-market data, readiness activation, a new or professional line-item model, hosted operation, human or screen-reader conformance, independent validation, market fit, screening alpha, or probability calibration.
...
Priority 7 accessibility remains open but is not the active next lane. Its remaining safe automated work follows the portable HTML action-policy repair; true zoom, forced colors, reduced motion, screen-reader, and independent-human evidence remain external until a suitable environment exists.

...
- PR engineering gate: `.github/workflows/commercial-research-beta.yml` runs the minimal read-only test, dashboard, research-render, wording, hygiene, and whitespace contract for pull requests to `main`; the checked-in workflow is local contract evidence until GitHub reports an actual completed run, and it is never human review.
...
35. **Implemented locally; hosted result pending:** one least-privilege GitHub Actions workflow now runs only for pull requests to `main` and executes the full pytest suite, dashboard smoke, all Personal Research route renders, public wording, generated-artifact hygiene, and whitespace checks. It has no schedule, push trigger, secrets, provider access, readiness rebuild, deployment, or artifact upload. The repository contract is tested locally; independent hosted CI is proven only after GitHub reports a completed run on the current draft revision, and human review remains a separate unproven gate.
...
The pull-request engineering gate improves **software-delivery maturity and independent reproducibility** by moving selected local checks into GitHub's PR boundary. Its checked-in configuration does not itself prove a successful hosted run or independent human review, and neither automated result can prove source rights, data freshness, reviewer adoption, commercial demand, or product-market fit.
...
| PR engineering evidence | workflow implemented locally | pull-request-only, least-privilege test and read-only product gate | completed GitHub Actions result on the current PR revision and independent human review |

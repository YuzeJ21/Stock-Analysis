# CRDO Single-Stock Research Report
...
- Valuation support: Blocked until trusted DCF inputs are ready; missing now: revenue, FCF margin.
...
- Next local step: Complete trusted fundamentals for CRDO; missing fields: revenue, FCF margin. Run `make focus-fundamentals TICKER=CRDO`, then use SEC staging or the manual fundamentals import workflow.
...
- Still locked: Blocked features: DCF, earnings, analyst estimates. Excluded features: portfolio. Unavailable sections are intentionally locked; missing data is not inferred.
- Trusted input: Trusted fundamentals such as revenue, free cash flow or margin, and shares outstanding.
- One-company pilot packet: `make trusted-data-pilot-packet TICKER=CRDO` is read-only; use it to inspect local file status, rejected-row checks, and the validate/preview/apply proof path before changing readiness.
- Data Health lane: Fundamentals / DCF Proof. Suggested local check: `make focus-fundamentals TICKER=CRDO`. Confirm with `make dcf-readiness && make readiness` before treating the lane as available.
- Next research step: Complete trusted fundamentals for CRDO; missing fields: revenue, FCF margin. Run `make focus-fundamentals TICKER=CRDO`, then use SEC staging or the manual fundamentals import workflow.
...
- Next proof: Complete trusted fundamentals for CRDO; missing fields: revenue, FCF margin. Run `make focus-fundamentals TICKER=CRDO`, then use SEC staging or the manual fundamentals import workflow.
- Stop rule: Blocked features: DCF, earnings, analyst estimates. Excluded features: portfolio. Unavailable sections are intentionally locked; missing data is not inferred.
...
- Next proof step: `make focus-fundamentals TICKER=CRDO` before reviewing company valuation.
- Withhold until proven: Blocked features: DCF, earnings, analyst estimates. Excluded features: portfolio. Unavailable sections are intentionally locked; missing data is not inferred.
- Manual check: Complete trusted fundamentals for CRDO; missing fields: revenue, FCF margin. Run `make focus-fundamentals TICKER=CRDO`, then use SEC staging or the manual fundamentals import workflow.
...
- Copy-only proof step: `make focus-fundamentals TICKER=CRDO`
...
## Executive Summary
- Bottom line: CRDO is in `Price/setup review only` mode. Use price and setup context only. Company valuation stays blocked until trusted fundamentals and DCF inputs exist.
...
- Do not infer: Blocked features: DCF, earnings, analyst estimates. Excluded features: portfolio. Unavailable sections are intentionally locked; missing data is not inferred.
- Next step: Complete trusted fundamentals for CRDO; missing fields: revenue, FCF margin. Run `make focus-fundamentals TICKER=CRDO`, then use SEC staging or the manual fundamentals import workflow.
...
- Proof command: `make focus-fundamentals TICKER=CRDO` before treating the next layer as available.
...
- Ticker: CRDO
- Asset type: company
- Current role: Core Compounder. Test whether trend, fundamentals, and DCF support the long-duration thesis; current state is Extended.
...
## Setup / Momentum
- Compounder setup: Extended; final state: Extended. Review the readiness sections below before drawing conclusions.
- 1M performance: 17.7%
- 3M performance: 76.1%
- 1Y performance: 261.3%
- ATR / volatility: Not available; missing values stay visible instead of guessed.

## Risk Notes
- Risk watchpoint: setup is extended; avoid over-interpreting momentum without a pullback or consolidation context.
...
## Valuation Readiness
- DCF status: blocked.
- DCF missing inputs: revenue, FCF margin.
- Why DCF is blocked: missing revenue, FCF margin.
...
- Relative valuation: withheld until trusted fundamentals and DCF readiness pass; available peer context is held back until the company DCF gate is ready (peer status=calculated; peer count=3).
...
- Missing now: revenue, FCF margin.
...
- Missing revenue: sets the operating scale used for forecast assumptions when direct free cash flow is not enough by itself. Proof path: use `make focus-fundamentals TICKER=CRDO`, then `make sec-stage TICKERS=CRDO` or trusted manual fundamentals import.
...
- Safe sequence: `make focus-fundamentals TICKER=CRDO` -> stage SEC or trusted manual fundamentals rows -> `make imports-validate` -> `make imports-preview` -> `make imports-apply` -> `make dcf-readiness`.
...
- Trusted input path: resolve fundamentals / DCF first, then use `make focus-peers TICKER=CRDO` if peer context is still needed.
- Peer ladder: paused behind core company readiness.
- Mapping evidence: mapping status=mapped; peer count=3. Do not use peer rows to bypass missing price, fundamentals, or DCF inputs.
...
- Next safe command: `make focus-peers TICKER=CRDO` only after the core DCF blockers are resolved.
...
- Rebuild proof: run `make optional-context-readiness`, then `make stock-report-md TICKER=CRDO` to confirm optional sections changed from locked to available.
...
## Missing Data
- Equity value could not be derived because cash/debt or net debt is unavailable.
- No trusted analyst-estimate CSV has been added yet.
- No trusted earnings CSV has been added yet.
- Peer input still missing: price/free-cash-flow unavailable for peer(s) ALAB, MRVL.
- Peer input still missing: P/E unavailable for peer(s) ALAB, MRVL.
- Revenue is unavailable from the current local fundamentals dataset.
- Valuation input still missing: EBITDA.
- Valuation input still missing: cash or debt.
...
- local:fundamentals.csv: research-grade / local; source readiness: dataset row as of 2025-05-03; Local fundamentals data.; Dataset row source: sec_companyfacts
...
- Fundamentals / DCF proof path: Fundamentals / DCF are blocked: missing revenue, FCF margin. Inspect `make focus-fundamentals TICKER=CRDO`, then use `make sec-stage TICKERS=CRDO` when SEC_USER_AGENT is configured or prepare trusted manual fundamentals rows before `make imports-validate`, `make imports-preview`, `make imports-apply`, and `make dcf-readiness`.
...
- Inspect this ticker: `make stock-report-md TICKER=CRDO`.
- One-company proof packet: `make trusted-data-pilot-packet TICKER=CRDO`.
- Price source readiness: `make focus-price TICKER=CRDO`.
- Fundamentals / DCF: `make focus-fundamentals TICKER=CRDO`.
- SEC/manual import checklist: `make sec-stage-queue TICKERS=CRDO TOP_N=10`.
...
- Peer review: `make focus-peers TICKER=CRDO`.
- Optional context checklist: `make optional-context-worklist TICKERS=CRDO TOP_N=10`.
...
- Report command: `make stock-report-md TICKER=CRDO`. Research-only Markdown output; copyable command only.

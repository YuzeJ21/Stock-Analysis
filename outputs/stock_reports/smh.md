...
- Copy-only proof step: `make stock-report-md TICKER=SMH`
...
- Data-confidence explanation: Data confidence is medium: market, theme, liquidity, or risk context may be reviewable, while company valuation is excluded.
...
- Rebuild proof: run `make optional-context-readiness`, then `make stock-report-md TICKER=SMH` to confirm optional sections changed from locked to available.
...
- Peers: monitor context; local source `data/peers.csv`; import target `data/imports/peers.csv`; next peer action No peer import is required; operating-company peer valuation is excluded for ETF/index/fund monitor context.

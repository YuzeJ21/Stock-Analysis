...
- Next proof step: `make focus-fundamentals TICKER=APLD` before reviewing company valuation.
...
- Proof command: `make focus-fundamentals TICKER=APLD` before treating the next layer as available.
...
- Fundamentals / DCF proof path: Fundamentals / DCF are blocked: missing free cash flow, shares outstanding, revenue, FCF margin. Inspect `make focus-fundamentals TICKER=APLD`, then use `make sec-stage TICKERS=APLD` when SEC_USER_AGENT is configured or prepare trusted manual fundamentals rows before `make imports-validate`, `make imports-preview`, `make imports-apply`, and `make dcf-readiness`.

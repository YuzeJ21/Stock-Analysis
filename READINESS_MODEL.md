...
| `review_metrics_ready` | prices, benchmark prices, fundamentals, peers | enough aligned ticker/benchmark rows for benchmark metrics; trusted fundamentals/peer inputs for deeper metrics | numeric local rows only, benchmark alignment required, risk-free-rate assumption explicit | stock report, dashboard Snapshot, `make benchmark-risk-review` | show benchmark/risk, fundamentals, valuation, and peer review metrics with source context | show available subset and exact missing inputs | block missing benchmark, fundamentals, market-cap, or peer inputs | exclude operating-company metrics for unsupported asset types where applicable |

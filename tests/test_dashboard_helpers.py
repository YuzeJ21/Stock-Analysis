...
def test_single_stock_source_json_label_uses_visitor_friendly_language():
    ...
    assert "Show data source details" in source
    assert "Show source readiness details" not in source
    assert "Adds extra data-source and missing-input checks under Sources & Gaps" in source
...
def test_dashboard_first_read_copy_avoids_engineering_queue_language():
    ...
    assert "Start here to see which DCF-ready companies still need source-backed peer rows." in source
...
def test_overview_landing_cards_surface_workflow_and_gap_context():
    ...
    assert len(cards) == 4
    assert "12 names checked" in rendered
    assert "12 stocks checked" not in rendered
        ...
        [
            {
                ...
                "Local File Status": "Local file status: peer import 1 data row(s); rejected-row report missing. File presence is not proof. Peer rows still require source-backed relationship review and readiness proof.",
            ...
            },
            {
                ...
                "Local File Status": "Local file status: fundamentals import 1 data row(s); staged fundamentals 0 file(s); rejected-row report present. File presence is not proof. Rows still require source review, validate, preview, apply, and readiness proof.",
...
def test_readiness_product_cards_use_plain_missing_output_language():
    ...
    assert "run readiness to rebuild the peer proof" in rendered
...
def test_readiness_recent_progress_cards_show_current_only_baseline_without_prior_snapshot():
    ...
    assert "latest refresh timestamp" in rendered
...
def test_first_fundamentals_unlock_frame_prefers_manual_path_without_sec_user_agent():
    ...
    assert "before report, exact missing input, lane-specific review command, validate/apply step, rejected-row report, and rebuild proof" in rendered

...
from src.decision_proof_queue import (
    ...
    build_decision_proof_queue_operator_flow_cards,
...
def test_decision_proof_queue_operator_flow_blocks_stale_rows():
    freshness = FreshnessStatus(
        "stale",
        "Research decision output is older than the current ticker readiness report. Run make research-decisions before building the proof queue.",
        "make research-decisions",
    ...
    cards = build_decision_proof_queue_operator_flow_cards(pd.DataFrame(), freshness)
    rendered = " ".join(str(value) for card in cards for value in card.values()).lower()
...
def test_decision_proof_queue_operator_flow_shows_current_next_step():
    queue = pd.DataFrame(
        [
            {
                "ticker": "META",
    ...
    cards = build_decision_proof_queue_operator_flow_cards(queue, freshness)
    rendered = " ".join(str(value) for card in cards for value in card.values()).lower()

    assert [card["kicker"] for card in cards] == ["FRESHNESS GATE", "NEXT REVIEW", "PROOF BOUNDARY"]
    assert cards[1]["command"] == "make focus-peers TICKER=META"
    assert "queue current" in rendered
    assert "review now: standalone dcf scenario analysis can be reviewed" in rendered
    assert "locked: peer-relative valuation stays locked" in rendered
    assert "prove before reading deeper" in rendered
    assert "buy" not in rendered
    assert "sell" not in rendered


def test_decision_proof_queue_operator_flow_handles_current_empty_queue():
    freshness = FreshnessStatus("current", "Readiness artifacts are current relative to watched source files.")

    cards = build_decision_proof_queue_operator_flow_cards(pd.DataFrame(), freshness)
    rendered = " ".join(str(value) for card in cards for value in card.values()).lower()

    assert cards[0]["title"] == "Snapshot current"

...
def test_render_trusted_data_pilot_candidates_is_read_only_and_actionable(tmp_path):
    ...
    assert "8. Rebuild lane proof: make readiness-snapshot PROFILE=<default|demo|local>" in rendered
...
def test_render_trusted_data_pilot_packet_prints_one_company_proof_loop(tmp_path):
    ...
    assert "1. Baseline readiness: make readiness-snapshot PROFILE=<default|demo|local>" in rendered
...
def test_load_trusted_data_pilot_candidates_skips_reviewed_non_actionable_fundamentals(tmp_path):
    _write_text(
        tmp_path / "outputs" / "fundamentals_peer_worklist.csv",
        "ticker,priority,dcf_ready,missing_required_for_dcf,focus_command\n"
...
def test_render_trusted_data_pilot_board_summarizes_batch_without_writing_files(tmp_path):
    ...
    assert "proof_command_pattern: make readiness-snapshot PROFILE=<default|demo|local>" in rendered
    assert "make reviewed-batch-compare PROFILE=<default|demo|local> LANE=fundamentals" in rendered
...
def test_render_trusted_data_pilot_lane_prints_ordered_evidence_summary_without_writing_files(tmp_path):
    ...
    assert "Command that confirms readiness changed: make readiness-snapshot PROFILE=<default|demo|local>" in rendered
    assert "make reviewed-batch-compare PROFILE=<default|demo|local> LANE=fundamentals" in rendered

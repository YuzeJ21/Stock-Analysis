...
def test_input_identity_contract_names_only_config_and_ten_profile_inputs():
    assert READINESS_SOURCE_FILENAMES == (
...
@pytest.mark.parametrize("name", READINESS_SOURCE_FILENAMES)
def test_input_identity_changes_when_each_exact_named_input_changes(tmp_path: Path, name: str):
    data_dir = _make_profile(tmp_path)
    before = readiness_input_identity(tmp_path, "default")
    path = tmp_path / name if name.startswith("config/") else data_dir / name
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(f"exact bytes for {name}\n".encode())

    after = readiness_input_identity(tmp_path, "default")

    assert len(before) == 64
    assert len(after) == 64
    assert after != before
    assert readiness_input_identity(tmp_path, "default") == after


def test_input_identity_binds_profile_name_and_normalized_relative_paths(tmp_path: Path):
    identities = {}
    for profile in ("default", "demo", "local"):
        data_dir = _make_profile(tmp_path, profile)
        (data_dir / "universe_master.csv").write_bytes(b"ticker\nAAA\n")
        identities[profile] = readiness_input_identity(tmp_path / ".", profile)

    assert len(set(identities.values())) == 3


def test_unrelated_generated_report_does_not_change_input_identity(tmp_path: Path):
    data_dir = _make_profile(tmp_path)
    before = readiness_input_identity(tmp_path, "default")
    reports = data_dir / "reports"
    reports.mkdir()
    (reports / "ticker_readiness_report.csv").write_bytes(b"unrelated generated bytes\n")

    assert readiness_input_identity(tmp_path, "default") == before


@pytest.mark.parametrize("target_scope", ["inside", "outside"])
@pytest.mark.parametrize("component", ["config", "readiness.yml"])
def test_input_identity_rejects_config_symlinks_before_reading(
    tmp_path: Path, component: str, target_scope: str
):
    _make_profile(tmp_path)
    config = tmp_path / "config"
    target_root = tmp_path if target_scope == "inside" else tmp_path.parent
    if component == "config":
        target = target_root / f"config-target-{target_scope}"
        target.mkdir(exist_ok=True)
        config.symlink_to(target, target_is_directory=True)
    else:
        config.mkdir()
        target = target_root / f"readiness-target-{target_scope}.yml"
        target.write_bytes(b"price_ready: {}\n")
        (config / "readiness.yml").symlink_to(target)

    with pytest.raises(ReadinessSourceBoundaryError, match="symbolic link"):
        readiness_input_identity(tmp_path, "default")

...
def test_methodology_doc_explains_formulas_limits_and_code_paths():
    ...
    for phrase in (
        ...
        "0.55 to below 0.80",
        "0.25 to below 0.55",
        "Decision confidence is capped by decision bucket",
        "Blocked by Data",
        "Stays low even if some partial context exists",
        "an ETF/index monitor row can have low or medium confidence for monitoring while DCF stays excluded",
...
def test_dashboard_advanced_commands_recommend_dry_run_before_refresh():
    ...
    assert "Show Local Research Report" in dashboard

...
from src.license_status import (
...
def test_render_license_status_names_owner_decision_and_stop_rule(tmp_path: Path):
    ...
    assert "- Controlled portfolio showcase | Keep the current controlled demo license | Visitors can review the project, but reuse rights are not granted." in rendered
...
def test_build_license_status_with_root_license_reports_controlled_demo_boundary(tmp_path: Path):
    (tmp_path / "LICENSE").write_text("Controlled Portfolio Demo License\n", encoding="utf-8")

    status = build_license_status(tmp_path)
    rendered = render_license_status(status)

    assert status["license_present"] is True
    assert status["share_status"] == "controlled_demo_license"
    assert status["owner_decision_required"] is False
    assert status["safe_to_share_boundary"] == CONTROLLED_DEMO_SHARE_BOUNDARY
    assert "share_status: controlled_demo_license" in rendered
    assert "do not describe as open source or reusable software" in rendered
    assert "copying, redistribution, sublicensing, hosted reuse" in rendered

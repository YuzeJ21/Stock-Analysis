...
def _write_monthly_fixture(base: Path) -> None:
    ...
    (base / "outputs" / "final_watchlist.csv").write_text(
        ...
        "CCC,Semis,SMH,Momentum Leader,No Setup,Review Thesis,ready,42,3,Needs review,Transparent reason\n"
        "DDD,Fintech,ARKF,Re-rating / Undervalued,Watch,Watch,ready,61,4,Watchlist context,Transparent reason\n"
        "EEE,Software,QQQ,Core Compounder,Extended,Extended,ready,58,5,Extended,Transparent reason\n"
    ...
    (base / "outputs" / "momentum_leaders.csv").write_text(
        "Ticker,Return1M,Return3M,Return6M,Return12M,RSPercentile,SetupStatus,Close,EMA10,EMA21,SMA50,SMA200,AvgVolume20D,VolumeRatio,ATRorVolatilityPct,ATRorVolatilitySource\n"
        "AAA,0.15,0.3,0.5,0.8,96,Watch,130,125,120,110,100,1000000,1.4,0.04,atr\n"
        "BBB,0.08,0.12,0.2,0.3,80,Setup Forming,105,103,101,99,95,400000,1.0,0.03,volatility_proxy\n"
        "CCC,,,,,No Setup,,,,,,,,,,\n"
        "DDD,0.04,,,,70,Watch,50,49,48,,,250000,0.9,0.05,volatility_proxy\n"
        "EEE,0.02,0.05,,,65,Extended,75,70,68,60,,100000,0.7,0.09,atr\n"
        "FFF,-0.2,-0.3,-0.4,-0.5,5,Broken,10,12,13,15,20,50000,0.4,0.12,atr\n",
        encoding="utf-8",
    )
...
def test_monthly_picks_generates_top_five_with_reasons_and_missing_data(tmp_path: Path):
    ...
    assert frame["Reason"].str.contains("Risk penalty").all()
    assert frame["Reason"].str.contains("volatility proxy approximation").any()

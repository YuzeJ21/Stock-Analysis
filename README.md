...
| Fundamentals / DCF | Use `DRY_RUN=1 make fundamentals-batch-proof TOP_N=10` for a reviewed SEC/manual proof packet preview, or inspect one company with `make trusted-data-pilot-packet TICKER=CRDO` before staging rows. | Company valuation only appears after required fields, validation, preview, rejected-row review, and readiness proof pass. |

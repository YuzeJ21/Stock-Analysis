...
## Pilot Share Status
Share as portfolio/demo evidence only; do not describe the repository as open source until a root `LICENSE` exists. Generated CSV/JSON/report churn stays local unless an exact artifact is reviewed as evidence. When source-proof queues are exhausted, use `make project-status` -> `make provider-setup-checklist` -> one reviewed one-ticker smoke command. No broad coverage batch should run from setup alone.

...
| Fundamentals / DCF | Use `make dcf-input-proof-queue TOP_N=25` to see whether DCF is blocked by shares outstanding, revenue, free cash flow, FCF margin, price, or an input bundle; then use `make dcf-input-source-review FAMILY=shares_outstanding TOP_N=10` and `make dcf-input-source-guard ...` before `make dcf-input-proof-handoff FAMILY=shares_outstanding TOP_N=10` or `DRY_RUN=1 make fundamentals-batch-proof TOP_N=10`. | Company valuation only appears after required source fields, validation, preview, rejected-row review, apply decision, and readiness proof pass. |

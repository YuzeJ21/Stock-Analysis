...
First review move: open Stock Selector, choose a ticker such as `NVDA`, read the Single-Stock Report answer, then open Data Health only when an input is blocked. The selected-profile trust strip follows [Data Profiles](docs/DATA_PROFILES.md) and shows source/readiness dates, identity, freshness, and matching counts. The local Change Monitor compares explicit same-profile snapshots and queues unresolved evidence as research tasks, never rankings or recommendations. The append-only Research Thesis Journal preserves reviewer-authored hypotheses, evidence, invalidation conditions, confidence history, and review dates without changing readiness. The readiness-gated, session-local Scenario Lab lets a reviewer vary bounded DCF assumptions without writing canonical data or producing a recommendation; see [Methodology](docs/METHODOLOGY.md) and the [Operator Guide](docs/OPERATOR_GUIDE.md).
## What It Does
